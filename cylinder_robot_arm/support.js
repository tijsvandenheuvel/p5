function createUIElements() {
	...
	checkbox_orbit = createCheckbox("orbit control", false);
	checkbox_orbit.changed(myCheckedEvent5);
	checkbox_orbit.position((windowWidth * 2) / 3 + 10, 130);

	// creeer checkbox om kleur aan en uit te zetten
	checkbox_color = createCheckbox("fill colors", true);
	checkbox_color.changed(myCheckedEvent2);
	checkbox_color.position((windowWidth * 2) / 3 + 10, 150);
	...
	checkbox_lines.position((windowWidth * 2) / 3 + 10, 170);

	// creeer checkbox om punten / bollen aan en uit te zetten
	checkbox_points = createCheckbox("draw points", false);
	checkbox_points.changed(myCheckedEvent4);
	checkbox_points.position((windowWidth * 2) / 3 + 10, 190);
...
function myCheckedEvent1() {
	if (this.checked()) {
		runsim_checkbox_value = true;
	} else {
		runsim_checkbox_value = false;
	}
}

function myCheckedEvent2() {
	if (this.checked()) {
		color_checkbox_value = true;
	} else {
		color_checkbox_value = false;
	}
}

function myCheckedEvent3() {
	if (this.checked()) {
		lines_checkbox_value = true;
	} else {
		lines_checkbox_value = false;
	}
}

function myCheckedEvent4() {
	if (this.checked()) {
		points_checkbox_value = true;
	} else {
		points_checkbox_value = false;
	}
}

function myCheckedEvent5() {
	if (this.checked()) {
		orbit_checkbox_value = true;
	} else {
		orbit_checkbox_value = false;
	}
}